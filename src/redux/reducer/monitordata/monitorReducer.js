...
import {MonitorData} from '../../../api/BackEndURL'
import { AuditMonitor } from "../../../api/BackEndURL";
let id=''
export const MonitorDataGet = createAsyncThunk("MonitorDataGet", async () => {
    return await MonitorData.method.get(MonitorData.URL.get).then((response) => response.data)
})

export const MonitorGetOne = createAsyncThunk("MonitorGetOne", async () => {
    return await MonitorData.method.get(`${MonitorData.URL.get}${id}`).then((response) => response.data)
})
export const AuditMonitorGet = createAsyncThunk("AuditMonitorGet", async () => {
    return await AuditMonitor.method.get(AuditMonitor.URL.get).then((response) => response.data)
})
