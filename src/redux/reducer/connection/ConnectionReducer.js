...
import {Connection} from '../../../api/BackEndURL'
...
export const connectionGet = createAsyncThunk("ConnectionGet", async () => {
    return await Connection.method.get(Connection.URL.get).then((response) =>response.data)
})
export const connectionAdd=createAsyncThunk("ConnectionAdd",async(Details)=>{
    return await Connection.method.post(`${Connection.URL.post}`,Details).then((response)=>response.data)
})
export const connectionGetId=createAsyncThunk("ConnectionGetID",async()=>{
    return await Connection.method.get(`${Connection.URL.getOne}${id}/`).then((response)=>response.data)
})
export const connectionPutId=createAsyncThunk("connectionPutId",async(Detail)=>{
    return await Connection.method.put(`${Connection.URL.put}${id}/`,Detail).then((response)=>response.data)
})
