...
import { Audit } from '../../../api/BackEndURL'
import { message } from 'antd'

let id = ''
let key = 'updatable'
export const AuditSearchGet = createAsyncThunk("AuditSearchGet", async () => {
    return await Audit.method.get(Audit.URL.get).then((response) => response.data)
})
...
export const AuditGetsheettwo = createAsyncThunk("AuditGetsheettwo", async (Data) => {
    return await Audit.method.put(Audit.URL.get).then((response) => response.data)
})
