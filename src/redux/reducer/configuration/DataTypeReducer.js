...
import { DataType } from '../../../api/BackEndURL'
import { message } from 'antd'

let id = ''
let key = 'updatable'
export const DataTypeGet = createAsyncThunk("DataTypeGet", async () => {
    return await DataType.method.get(DataType.URL.get).then((response) => response.data)
})

export const DataTypePost = createAsyncThunk("DataTypeAdd", async (Data) => {
    return await DataType.method.post(`${DataType.URL.post}`, Data).then((response) => response.data)
})

export const DataTypeGetOne = createAsyncThunk("DataTypeGetOne", async () => {
    return await DataType.method.get(`${DataType.URL.get}${id}`).then((response) => response.data)
})

export const DataTypePut = createAsyncThunk("DataTypePut", async (Data) => {
    return await DataType.method.put(`${DataType.URL.put}${id}`, Data).then((response) => response.data)
})
